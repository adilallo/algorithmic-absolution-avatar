...
2. **Port the CRT render-layer into the production kiosk (`index.html`) — NEW, found round 3.** The oracle GLB is now the production avatar, but `index.html` renders it **raw/ungraded** — the blue/scanline/vignette/grain/bloom + backlit halo + telephoto head-and-shoulders framing all live only in `oracle-preview.html` (§5). To actually ship the "CRT oracle" look, lift that render-layer (the `EffectComposer` chain, the `head.render` monkeypatch, the rim light, the camera framing, and the `window.ORACLE` knobs baked to their tuned values) from `oracle-preview.html` into `index.html`, behind the production path. The treatment is base-independent and the Pi half-res-bloom optimization is already in the preview, so this is mostly a copy-port + re-verify 60fps. Decide with the artist whether dev mode (controls visible) should also show the grade.
